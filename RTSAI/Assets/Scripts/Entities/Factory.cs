...
public sealed class Factory : BaseEntity
{
    ...
    Queue<(int UnitIndex, Action<Unit> Callback)> BuildingQueue = new Queue<(int, Action<Unit>)>();
    ...
    public EFactoryWeightClass WeightClass { get { return FactoryData.WeightClass; } }
    public bool IsLight { get { return WeightClass == EFactoryWeightClass.Light; } }
    public bool IsHeavy { get { return WeightClass == EFactoryWeightClass.Heavy; } }
    ...
    private void Defend() 
    { 
        if (Controller.gameObject.GetComponent<AIController>())
        {
            Objective o = new Objective(Controller.gameObject.GetComponent<AIController>().DefendGoal, new Target(this), 20);

            Controller.gameObject.GetComponent<AIController>().objectives.Add(o);
        }
    }
        ...
        switch (CurrentState)
        {
            ...
            case State.BuildingUnit:
                if (Time.time > EndBuildDate)
                {
                    ...
                    {
                        (int unitIndex, Action<Unit> callback) = BuildingQueue.Dequeue();
                        StartBuildUnit(unitIndex, callback);
                    }
                }
                else if (BuildGaugeImage)
    ...
    {
        int counter = 0;
        foreach ((int UnitIndex, Action<Unit> Callback) entry in BuildingQueue)
        {
    ...
    public bool RequestUnitBuild(int unitMenuIndex, Action<Unit> _onUnitBuilt = null)
    {
        ...
        StartBuildUnit(unitMenuIndex, _onUnitBuilt);

        return true;
    }
    void StartBuildUnit(int unitMenuIndex, Action<Unit> _onUnitBuilt = null)
    {
        ...
        if (CurrentState == State.BuildingUnit)
        {
            if (BuildingQueue.Count < MaxBuildingQueueSize)
                BuildingQueue.Enqueue((unitMenuIndex, _onUnitBuilt));
            return;
        }
        ...
        if (_onUnitBuilt != null)
            OnUnitBuilt += _onUnitBuilt;
    }
    ...
    public void CancelCurrentBuild()
    {
        ...
        foreach ((int UnitIndex, Action<Unit> Callback) entry in BuildingQueue)
        {
            Controller.TotalBuildPoints += GetUnitCost(entry.UnitIndex);
        }
        BuildingQueue.Clear();
