...
public sealed class PlayerController : UnitController
{
    ...
    override protected void Start()
    {
        ...
        PlayerMenuController.SquadAgressivenessToggle.onValueChanged.AddListener(UpdateSquadAgressiveness);
    }
    ...
    private void UpdateSquadAgressiveness(bool _isAgressive)
    {
        SelectedSquad.SetAgressiveness(_isAgressive);
    }
    #endregion
    ...
    void GiveSquadOrder(GoapGoal _goal, Target _target)
    {
        if (SelectedUnitList.Count <= 0)
            return;
        if (SelectedUnitList[0].Squad != null)
            SelectSquad(SelectedUnitList[0].Squad);

        if (SelectedSquad == null || !SelectedSquad.Equals(SelectedUnitList))
        {
            SpawnSquad(SelectedUnitList);
            Unit closestUnit = GameServices.GetClosestUnitToObjective(SelectedUnitList, _target.PositionTarget);
            if (closestUnit != null)
                SelectedSquad.transform.position = closestUnit.transform.position;
        }
        else
        {
            foreach (Unit unit in SelectedUnitList)
            {
                if (unit.Squad != SelectedSquad)
                    SelectedSquad.AddUnit(unit);
            }
        }
        SelectedSquad.UpdateObjective(_goal, _target);
    }
    ...
    void ComputeUnitsAction()
    {
        ...
        // Set unit/factory attack/repair/defend target and goal
        if (Physics.Raycast(ray, out raycastInfo, Mathf.Infinity, damageableMask))
        {
            ...
            if (other != null)
            {
                ...
                {
                    GiveSquadOrder(DestroyGoal, new Target(other));
                }
                else if (other.NeedsRepairing())
                {
                    GiveSquadOrder(RepairGoal, new Target(other));
                }
                else
                {
                    GiveSquadOrder(DefendGoal, new Target(other));
                }
            }
        }
        // Set capturing target and goal
        else if (Physics.Raycast(ray, out raycastInfo, Mathf.Infinity, targetMask))
        {
            ...
            {
                GiveSquadOrder(CaptureGoal, new Target(target));
            }
        }
        // Set squad move target and goal
        else if (Physics.Raycast(ray, out raycastInfo, Mathf.Infinity, floorMask))
        {
            ...
            GiveSquadOrder(ExploreGoal, new Target(newPos));
        }
    }

    protected override void SelectSquad(Squad _squad)
    {
        base.SelectSquad(_squad);
        PlayerMenuController.ShowSquadMenu(SelectedSquad.Units[0].IsAgressive);
        PlayerMenuController.SquadAgressivenessToggle.onValueChanged.AddListener(SelectedSquad.SetAgressiveness);
    }

    protected override void UnselectSquad()
    {
        base.UnselectSquad();
        PlayerMenuController.HideSquadMenu();
        PlayerMenuController.SquadAgressivenessToggle.onValueChanged.RemoveAllListeners();
    }
