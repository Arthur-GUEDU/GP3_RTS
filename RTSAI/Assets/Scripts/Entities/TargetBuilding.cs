    ...
    void OnCaptured(ETeam newTeam)
    {
        ...
        if (OwningTeam != newTeam)
        {
            ...
            if (OwningTeam != ETeam.Neutral)
            {
                ...
                if (teamController.GetComponent<AIController>())
                {
                    Objective o = new Objective(teamController.GetComponent<AIController>().CaptureGoal, new Target(this), 10);

                    teamController.GetComponent<AIController>().objectives.Add(o);
                }
            }
        }
